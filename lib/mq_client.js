...
class MQClient extends MQClientAPI {
  ...
  * handleServerRequest(request, address) {
    const command = request.data;
    if (!command) return;
    switch (command.code) {
      case RequestCode.CHECK_TRANSACTION_STATE:
        yield this.checkTransactionState(command, address);
        break;
      default:
        break;
    }
  ...
  * checkTransactionState(command, addr) {
    ...
    yield producer.checkTransactionState(addr, remoteCommand, requestHeader);
  }
