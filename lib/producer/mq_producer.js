...
class MQProducer extends ClientConfig {
  ...
  * sendMessageInTransaction(msg, tranExecuter, arg) {
    ...
    try {
      sendResult = yield this.send(msg);
    } catch (e) {
    ...
    switch (sendResult.sendStatus) {
      case 'SEND_OK':
        ...
        try {
          localTransactionState = yield tranExecuter(msg, arg);
        } catch (e) {
    ...
    try {
      yield this.endTransaction(sendResult, localTransactionState, localException);
    } catch (err) {
  ...
  * endTransaction(sendResult, localTransactionState, localException) {
    ...
    yield this._mqClient.endTransactionOneway(brokerAddr, requestHeader, remark);
  }

  * checkTransactionState(addr, msg, checkRequestHeader) {
    let localTransactionState;
    let exception;
    try {
      localTransactionState = yield this.localTransactionChecker(msg);
    } catch (error) {
    ...
    yield this._mqClient.endTransactionOneway(addr, thisHeader, remark);
  }
