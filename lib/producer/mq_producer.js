...
class MQProducer extends ClientConfig {
  ...
  set localTransactionChecker(checker) {
    this._localTransactionChecker = checker;
  }

  get localTransactionChecker() {
    return this._localTransactionChecker;
  }
  ...
  async sendMessageInTransaction(msg, tranExecuter, arg) {
    if (!this.localTransactionChecker) {
      throw new Error('[MQClientException].producer.localTransactionCheckListener is required');
    }
    if (tranExecuter === null) {
      throw new Error('[MQClientException] tranExecutor is required');
    }
    let sendResult = null;
    msg.properties[MessageConst.PROPERTY_TRANSACTION_PREPARED] = 'true';
    msg.properties[MessageConst.PROPERTY_PRODUCER_GROUP] = this.producerGroup;
    try {
      sendResult = await this.send(msg);
    } catch (e) {
      throw new Error('[MQClientException] send message Exception ', e);
    }
    let localTransactionState = 'UNKNOW';
    let localException = null;
    switch (sendResult.sendStatus) {
      case 'SEND_OK':
        if (sendResult.transactionId !== null && msg.properties['__transactionId__'] === null) { // eslint-disable-line
          msg.properties['__transactionId__'] = sendResult.transactionId; // eslint-disable-line
        }
        try {
          localTransactionState = await tranExecuter(msg, arg);
        } catch (e) {
          this.logger.info('executeLocalTransactionBranch exception', e);
        ...
        if (localTransactionState !== 'COMMIT_MESSAGE') {
          this.logger.info('executeLocalTransactionBranch return', localTransactionState);
          this.logger.info(msg);
        }
        break;
    ...
    try {
      await this.endTransaction(sendResult, localTransactionState, localException);
    } catch (err) {
      this.logger.warn('local transaction execute ' + localTransactionState + ', but end broker transaction failed', err);
    }
  ...
  async endTransaction(sendResult, localTransactionState, localException) {
    let id;
    if (sendResult.offsetMsgId != null) {
      id = MessageDecoder.decodeMessageId(sendResult.offsetMsgId);
    } else {
      id = MessageDecoder.decodeMessageId(sendResult.msgId);
    }
    const transactionId = sendResult.transactionId;
    const brokerAddr = this._mqClient.findBrokerAddressInPublish(sendResult.messageQueue.brokerName);
    const requestHeader = {
      transactionId,
      commitLogOffset: id.offset,
    };
    ...
    requestHeader.producerGroup = this.producerGroup;
    requestHeader.tranStateTableOffset = sendResult.queueOffset;
    requestHeader.msgId = sendResult.msgId;
    const remark = localException != null ? ('executeLocalTransactionBranch exception: ' + localException.toString()) : null;
    await this._mqClient.endTransactionOneway(brokerAddr, requestHeader, remark);
  }

  async checkTransactionState(addr, msg, checkRequestHeader) {
    let localTransactionState;
    let exception;
    try {
      localTransactionState = await this.localTransactionChecker(msg);
    } catch (error) {
      this.logger.error('Broker call checkTransactionState, but checkLocalTransactionState exception', error);
      exception = error;
    }
    ...
    const thisHeader = {
      commitLogOffset: checkRequestHeader.commitLogOffset,
      producerGroup: checkRequestHeader.producerGroup,
      tranStateTableOffset: checkRequestHeader.tranStateTableOffset,
      fromTransactionCheck: true,
      msgId: msg.properties[MessageConst.PROPERTY_UNIQ_CLIENT_MESSAGE_ID_KEYIDX] || msg.msgId,
      transactionId: checkRequestHeader.transactionId,
    };

    switch (localTransactionState) {
      case 'COMMIT_MESSAGE':
        thisHeader.commitOrRollback = MessageSysFlag.TransactionCommitType;
        break;
      case 'ROLLBACK_MESSAGE':
        thisHeader.commitOrRollback = MessageSysFlag.TransactionRollbackType;
        this.logger.warn('when broker check, client rollback this transaction, {}', thisHeader);
        break;
      case 'UNKNOW':
        thisHeader.commitOrRollback = MessageSysFlag.TransactionNotType;
        this.logger.warn('when broker check, client does not know this transaction state, {}', thisHeader);
        break;
      default:
        break;
    }
    const remark = exception != null ? ('checkLocalTransactionState exception: ' + exception) : null;
    await this._mqClient.endTransactionOneway(addr, thisHeader, remark);
  }
